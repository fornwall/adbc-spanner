...
**2. README actively misdescribes the driver** — `README.md:51–54` says partitioned execution
(`execute_partitions`/`read_partition`) is *not supported*, but it's fully implemented and
emulator-tested; a reader would skip the driver for exactly the parallel-scan use case it handles.
The Releasing section (~line 256) also still says `cargo release` publishes to crates.io and needs
a token, but `publish = false` is set; and the Usage snippet says `adbc-spanner = "0.1"` (crate is
0.5.0 and not consumable from crates.io while git-pinned).

**3. `cancel()` is silently lost between chunk fetches** — `src/runtime.rs:40`:
...
**4. Malformed values silently become NULL on the read path** — `src/conversion.rs`: parse
...
**5. `INSERT/UPDATE/DELETE ... THEN RETURN` rows are discarded** — `src/statement.rs:369`:
...
**6. SQL lexers miss GoogleSQL raw and triple-quoted strings** — `src/ddl.rs:51`: in `r'C:\'` the
...
**7. Statement hints defeat statement classification** — `first_keyword("@{HINT=X} UPDATE ...")`
...
**8. Identifier quoting is inconsistent and uses the wrong escape** — `qualified_table`
...
**9. Release job can attach unchecksummed wheels** — `libraries.yml:94`: the `release` job
...
**10. CI supply-chain hygiene** — the release-critical actions are pinned to mutable refs
...
**11. gRPC error fidelity** — `src/error.rs`: `ABORTED` (Spanner's routine "retry me" signal) maps
...
**12. Untested data-loss path** — the "re-enabling autocommit commits buffered DML" branch
...
**13. Emulator scripts fail open** — `scripts/with-emulator.sh:44–64`: both readiness loops fall
