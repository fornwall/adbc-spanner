...
- [x] **CON-1 (Medium)** — `block_on` panics (call and drop) when the driver is entered from an async context — `src/runtime.rs:123`, `src/conversion.rs:364`, `src/driver.rs:369`
  ...
  *Resolved:* both panics are real — reverting each half of the fix reproduces exactly the two quoted messages — and both are gone. **But the suggested mechanism turned out to be wrong, and was not used for the call half.** Probing every context a driver call can be reached from (evidence table below) shows `Handle::try_current()` is `Ok` in three contexts where blocking *works*, including `tokio::task::spawn_blocking` — the very workaround the proposed error message would have to recommend. Guarding on it would therefore have broken *working* code under the default multi-thread `#[tokio::main]`: `spawn_blocking(|| stmt.execute())` would start returning `InvalidState` telling the caller to use `spawn_blocking`. That is worse than the panic, so a "clean error" is not implementable this way, and no public Tokio API exposes the real predicate (`context::try_enter_blocking_region`, i.e. the `EnterRuntime` flag, is `pub(crate)`; `catch_unwind` on it was rejected too — it aborts under `panic = "abort"` and would print a panic + backtrace before every "clean" error).
  ...
  So the driver does not *predict* whether blocking is allowed; `runtime::block_on_bridged` picks, per `Handle::try_current().map(runtime_flavor)`, a construction that is legal in **every** context that flavour can present — no context → plain `block_on`; `MultiThread` → `block_in_place` (on a worker it hands the core to another thread, so the caller's runtime keeps full capacity; on a `spawn_blocking` thread it is a pass-through); anything else (`CurrentThread`, where `block_in_place` itself panics, and any future `#[non_exhaustive]` flavour) → a `std::thread::scope` thread, which carries no Tokio context and may therefore always block, and needs no `'static`. Both bridge sites go through it: `block_on_cancellable` (covering `conversion.rs`'s `SpannerBatchReader::next` and `BoundQueryBatchReader`) and `SpannerDatabase::connect`'s plain `block_on`. A panic from the operation's own future is still re-raised unchanged via `resume_unwind`. The net effect is that the driver *works* from async code rather than erroring — the caller's thread still blocks, which is inherent to a synchronous ADBC API and is what the caller asked for; `spawn_blocking` remains the documented recommendation, just no longer a requirement. Cost: `block_on_cancellable` now requires `Send`, which forced hoisting the per-table SQL clone out of `statistics.rs`'s scan closure (its higher-ranked `&PreparedTable` argument defeated the compiler's `Send` check — a rustc limitation, not a real violation; the clone happened per scan anyway).

  The **drop** half is fixed exactly as suggested, because there the same over-approximation is *safe*: `SharedRuntime` is now `Arc<DriverRuntime>`, a newtype holding `Option<Runtime>` and `Deref`-ing to `Runtime` (so no call site changed, and no `unsafe`/`ManuallyDrop`); its `Drop` takes the runtime out and calls `shutdown_background()` when `Handle::try_current()` succeeds, plain drop otherwise. A false positive there only skips the wait for idle worker threads — it cannot turn a working call into an error. (`shutdown_background` is `shutdown_timeout(0)`, whose zero-timeout early return precedes tokio's `try_enter_blocking_region` panic, so it is genuinely safe in an async context.)

  Covered by `src/runtime.rs` unit tests asserting a bridged call works from **all six** contexts in the table (the `spawn_blocking` rows are what pin down the rejected design), that cancellation still works on the scoped-thread path, and that the last-handle drop is panic-free in both flavours with a live task pending; plus the end-to-end `driver_entered_from_an_async_context_does_not_panic` in `tests/mock_spanner.rs`, which drives `new_connection` → `execute` → reader drain → drop against the mock server from inside both flavours (every one of those panicked before the fix). This is mitigation, not a cure — the root cause remains adbc_core's sync trait surface (UP-13). Documented user-facing in the new lib.rs "Calling from async code" crate-doc section and in CLAUDE.md's sync-over-async design point.
