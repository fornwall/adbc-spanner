...
**1. Malformed values silently become NULL on the read path** — `src/conversion.rs`: parse
...
**2. `INSERT/UPDATE/DELETE ... THEN RETURN` rows are discarded** — `src/statement.rs:369`:
...
**3. SQL lexers miss GoogleSQL raw and triple-quoted strings** — `src/ddl.rs:51`: in `r'C:\'` the
...
**4. Statement hints defeat statement classification** — `first_keyword("@{HINT=X} UPDATE ...")`
...
**5. Identifier quoting is inconsistent and uses the wrong escape** — `qualified_table`
...
**6. Release job can attach unchecksummed wheels** — `libraries.yml:94`: the `release` job
...
**7. CI supply-chain hygiene** — the release-critical actions are pinned to mutable refs
...
**8. gRPC error fidelity** — `src/error.rs`: `ABORTED` (Spanner's routine "retry me" signal) maps
...
**9. Untested data-loss path** — the "re-enabling autocommit commits buffered DML" branch
...
**10. Emulator scripts fail open** — `scripts/with-emulator.sh:44–64`: both readiness loops fall
