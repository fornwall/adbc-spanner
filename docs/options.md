...
| `adbc.connection.readonly` | boolean | `false` | yes, unless a manual transaction is active (`true`/`false`) | **Standard ADBC.** Reject all writes on this connection: DML, DDL and bulk ingest fail with `InvalidState`; queries still run. The flag is live — statements read it at execution time, so toggling it immediately affects statements that already exist. Changing it while a manual transaction is **active** (a first statement has fixed the transaction's kind) fails with `InvalidState` (the JDBC `setReadOnly` rule): commit or roll back first. Re-setting the current value is a no-op and always allowed. |
