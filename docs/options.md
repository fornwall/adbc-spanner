...
| `adbc.statement.bind_by_name` | boolean; `""` unsets | unset (auto-detect) | yes, when set (`true`/`false`); unset reads as `NotFound` | How bound Arrow columns pair with the query's `@name` parameters, following the ADBC SQLite reference driver's `bind_by_name` convention ([apache/arrow-adbc#3362](https://github.com/apache/arrow-adbc/issues/3362)). **Unset**: auto-detect — by name when *every* bound column names a query parameter, else positionally (*i*-th column → *i*-th distinct parameter in query order). **`true`**: strict by-name — a bound column that names no query parameter fails with `InvalidArguments` naming the missing parameter. **`false`**: strictly positional — column names are ignored entirely, closing the auto-detection pitfall where a positional client's column names all coincidentally match the parameters and silently bind name-matched (possibly reordered). See [README § Status](../README.md#status). |
