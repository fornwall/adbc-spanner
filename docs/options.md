...
| Option | Type / allowed values | Default | Round-trips | Description |
| ------ | --------------------- | ------- | ----------- | ----------- |
| `adbc.connection.autocommit` | boolean | `true` | yes, always (`true`/`false`) | **Standard ADBC.** `false` enters manual transaction mode: a transaction is exactly one kind of work — **queries** (all sharing one multi-use read-only snapshot), **DML** (buffered, applied atomically in one read/write transaction on `commit`; `execute_update` returns an unknown row count until then, and a query inside the transaction fails with `InvalidState` — no read-your-writes), or **DDL** (buffered, applied on `commit` as one `UpdateDatabaseDdl` batch) — fixed by its first statement; mixing kinds fails with `InvalidState`. `rollback` discards the buffered work / drops the snapshot. See [README § Status](../README.md#status) for the full caveats. Setting it back to `true` commits any buffered transaction (on failure the buffer is restored and the error returned). |
