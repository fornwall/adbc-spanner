...
| `spanner.access_token` | string: OAuth 2.0 bearer token | unset (Application Default Credentials) | yes, when set (returned verbatim, matching `spanner.keyfile_json`) | Authenticate with a caller-supplied OAuth 2.0 access token, sent verbatim as `Authorization: Bearer <token>` with **no refresh** (the caller owns token validity). A complete credential in its own right, so it is **mutually exclusive** with `spanner.keyfile`, `spanner.keyfile_json`, and `spanner.impersonate.target_principal` — combining it with any of them is refused at connect time with `InvalidState`. See [README § OAuth access token](../README.md#oauth-access-token). |
...
- **Application Default Credentials** — when no keyfile or `spanner.access_token` option is set
