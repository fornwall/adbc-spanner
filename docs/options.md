...
case-folded. (Free-form values such as tags are stored verbatim — surrounding whitespace is *not*
trimmed; only the grammar parsers for structured values, such as durations and timestamps, trim
around the parts they parse.)
...
> **Before `Init`, at the C ABI.** The table columns below describe the driver objects. Through a
> driver manager there is an extra phase: a database or connection option set *before*
> `AdbcDatabaseInit` / `AdbcConnectionInit` is only **buffered** (ordered, last-write-wins per key)
> and replayed through the driver's constructor at `Init` — so it is not validated when it is set,
> and a bad key or value surfaces as an error from `Init` rather than from `SetOption`. Symmetrically,
> **every** pre-`Init` get fails with `NotFound`, whatever the *Round-trips* column says, because the
> driver has not yet accepted the value. A statement is live from the moment it is created, so it has
> no such phase.
...
and `adbc.connection.db_schema`. Neither can be pointed at anything else — setting either to the
value it already reports is an accepted no-op (so generic clients that always set them keep
working), while any other value fails with `NotImplemented`. `adbc.connection.catalog` reports the
connection's **database id** (the `<d>` of `projects/<p>/instances/<i>/databases/<d>`), the one
catalog it can reach and the same name `get_objects` reports — a connection is bound to one database
for its whole life. `adbc.connection.db_schema` reports `""`: Spanner supports named schemas
(addressed by qualified name, e.g. `sales.Orders`, and enumerated by `get_objects`) but has no
settable session/current schema to select one.
...
| `spanner.transaction.exclude_from_change_streams` | boolean; `""` unsets | `false` | yes, always (`true`/`false`) | Exclude the transaction's writes from [change-stream capture](https://docs.cloud.google.com/spanner/docs/reference/rest/v1/TransactionOptions) (`exclude_txn_from_change_streams`). Applied at the same read/write commit sites as `spanner.commit.max_delay` — autocommit DML, the `ExecuteBatchDml` batch runner, the manual-mode commit, and the bulk-ingest write-only transaction — **and** on the `spanner.ingest.batch_write` firehose `BatchWrite` request (which carries the flag directly). Per Spanner this only takes effect for change streams created with the DDL option `allow_txn_exclusion = true`; other change streams record the writes regardless. |
...
| `spanner.retry.max_attempts` | positive integer (see [Retry tuning](#retry-tuning)) | unset (client default: uncapped on unary RPCs, 10 attempts on queries) | yes, when set (also via `get_option_int`) | Cap on the number of attempts (first try + retries) the client makes for a retryable RPC; `1` disables retrying. Bounds the client's default retry policy without dropping its transport-error-on-idempotent retrying. Exact on every RPC path — see [What the two limits actually deliver](#what-the-two-limits-actually-deliver-per-rpc-path). |
...
| `adbc.ingest.target_catalog` | the connection's database id only (what `adbc.connection.catalog` reports) | unset (the connection's own catalog) | yes, when set | **Standard ADBC.** An ingest cannot cross databases, so the connection's own catalog is the only accepted value; any other name — including `""`, `adbc.h`'s "no catalog" — fails with `NotImplemented`. |
