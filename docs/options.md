...
| `spanner.partitioned_dml` | boolean; `""` unsets | `false` (ordinary read/write DML) | yes, always (`true`/`false`) | Run an **autocommit** DML statement as Spanner [Partitioned DML](https://docs.cloud.google.com/spanner/docs/dml-partitioned) — large-scale, idempotent, **non-atomic** `UPDATE`/`DELETE` that Spanner automatically partitions and applies to each partition independently (retrying per partition) — instead of an ordinary read/write transaction. `execute_update` returns a **lower bound** on the rows modified (a partition may apply more than once, so the true count can be higher — Spanner reports only the lower bound). Because a partitioned statement runs its own implicitly-committed transaction and cannot be buffered or rolled back, it is only valid for a **single** autocommit statement: it is rejected in a **manual transaction** (`InvalidState`), with a **`THEN RETURN`** clause (`InvalidArguments` — Partitioned DML returns no rows), and for a **`;`-batch** or **bound parameters** (`InvalidArguments` — it executes exactly one statement). The statement must be a fully-partitionable `UPDATE`/`DELETE`; Spanner rejects anything else (e.g. `INSERT`) at execution time. `spanner.request.*`, `spanner.query.*`, `spanner.retry.*` and `spanner.rpc.timeout_seconds.update` still apply. |
